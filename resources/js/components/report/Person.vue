    ...
    <div class="flex flex-col w-full mt-11 mb-11">
      <table class="min-w-full border border-gray-300">
        <thead class="bg-gray-100">
          <tr>
          <th class="border px-4 py-3 text-center">บุคคล</th>
         ...
         <tbody>
          <template v-for="(item, i) in items">
            <tr class="hover:bg-green-50">
              <td class="border px-4 py-2">{{i+1}}. {{ item.name }}</td>
...
export default {
  ...
  methods: {
    ...
    showData(){
      this.isLoading = true;
      axios.get('/admin/report/person/load',{
        params: {
          type: this.s_type,
          month: this.s_month,
          year: this.s_year,
