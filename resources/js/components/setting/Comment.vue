  ...
  <div class="mt-6 mb-11">
      ...
      <table class=" w-full border border-[#dee2e6] text-left text-sm">
        <thead>
            <tr>
                ...
                <th width="60%" class="px-4 py-2 border border-[#dee2e6] text-center" scope="col">ประเด็นข้อคิดเห็น	</th>
          ...
          <tbody>
            <template v-for="(item) in items">
              <tr class="hover:bg-[#efefef]">
                ...
                <td class="px-4 py-2 border border-[#dee2e6] text-center">
                  <div class="flex justify-center">
                    <i @click="viewData(item.id, item.name, 'main')" class="far fa-edit text-[#007bff] hover:cursor-pointer"></i>
                  </div>
                </td>
              </tr>
              <tr v-for="(sub) in item.sub" class="hover:bg-[#efefef]">
                ...
                <td class="px-4 py-2 border border-[#dee2e6] text-center">
                  <div class="flex justify-center">
                    <i @click="viewData(sub.id, sub.name, 'sub')" class="far fa-edit text-[#007bff] hover:cursor-pointer"></i>
                  </div>
                </td>
              </tr>
            </template>
    ...
    >
      ...
      <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative animate-fade-in-down">
        <h2 class="text-xl font-semibold mb-4 text-[#EE7530]">แก้ไข ประเด็นข้อคิดเห็น</h2>
        <Form :initial-values="form" :validation-schema="schema" @submit="saveData">
          <div class="flex flex-col mt-3">
            <div class="text-sm">ประเดินความคิดเห็น : </div>
            <div class="gap-1">
...
export default {
  ...
  methods: {
    ...
    showData(){
      this.isLoading = true;
      axios.get('/admin/setting/comment/load') 
      .then(res => {
    ...
    saveData(values){
      this.isLoading = true;
      axios.post('/admin/setting/comment',{
        data: values,
      }) 
      .then(res => {
        if(res.data.status == 200){
